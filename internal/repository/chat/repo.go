...
func (r *chatRepository) Create(chat *chatdomain.Chat) error {
	_, err := r.db.Exec(
...
func (r *chatRepository) FindByID(id string) (*chatdomain.Chat, error) {
	row := r.db.QueryRow(
...
func (r *chatRepository) FindByUserID(userID string) ([]*chatdomain.Chat, error) {
	rows, err := r.db.Query(
...
func (r *chatRepository) Update(chat *chatdomain.Chat) error {
	_, err := r.db.Exec(
		`UPDATE chats SET name=?, description=?, avatar_url=?, slow_mode_seconds=?, updated_at=? WHERE id=?`,
		chat.Name, chat.Description, chat.AvatarURL, chat.SlowModeSeconds, time.Now().Format(time.RFC3339), chat.ID,
	)
	return err
}
...
func (r *chatRepository) GetParticipants(chatID string) ([]*chatdomain.ChatParticipant, error) {
	rows, err := r.db.Query(
	...
	participants := make([]*chatdomain.ChatParticipant, 0)
	for rows.Next() {
		var (
			p        chatdomain.ChatParticipant
			joinedAt string
			lastRead string
		)
		...
		p.LastReadAt = repository.ParseTime(lastRead)
		participants = append(participants, &p)
	}
	return participants, nil
}

func (r *chatRepository) GetParticipantsByChatIDs(chatIDs []string) (map[string][]*chatdomain.ChatParticipant, error) {
	if len(chatIDs) == 0 {
		return make(map[string][]*chatdomain.ChatParticipant), nil
	}
	...
	result := make(map[string][]*chatdomain.ChatParticipant)
	for rows.Next() {
		var (
			p        chatdomain.ChatParticipant
			joinedAt string
...
func (r *chatRepository) GetPrivateChat(user1ID, user2ID string) (*chatdomain.Chat, error) {
	row := r.db.QueryRow(
		`SELECT c.id, c.name, COALESCE(c.description,''), c.avatar_url, c.type, c.created_by, c.created_at, c.updated_at
		FROM chats c
...
func (r *chatRepository) SetNotificationMuted(userID, chatID string, muted bool) error {
	_, err := r.db.Exec(
		...
		userID, chatID, repository.BoolToInt(muted),
	)
	return err
}
...
func (r *chatRepository) SearchByName(userID, query string) ([]*chatdomain.Chat, error) {
	rows, err := r.db.Query(
		`SELECT c.id, c.name, COALESCE(c.description,''), c.avatar_url, c.type, c.created_by, c.created_at, c.updated_at
		FROM chats c
		INNER JOIN chat_participants cp ON cp.chat_id = c.id
...
func (r *chatRepository) FindByUserIDExcludeHidden(userID string) ([]*chatdomain.Chat, error) {
	rows, err := r.db.Query(
		`SELECT c.id, c.name, COALESCE(c.description,''), c.avatar_url, c.type, c.created_by, c.created_at, c.updated_at
		FROM chats c
		INNER JOIN chat_participants cp ON cp.chat_id = c.id
...
func (r *chatRepository) FindByUserIDArchived(userID string) ([]*chatdomain.Chat, error) {
	rows, err := r.db.Query(
		`SELECT c.id, c.name, COALESCE(c.description,''), c.avatar_url, c.type, c.created_by, c.created_at, c.updated_at
		FROM chats c
...
func scanChat(row repository.Scanner) (*chatdomain.Chat, error) {
	var (
		c         chatdomain.Chat
		createdAt string
		updatedAt string
	...
	c.CreatedAt = repository.ParseTime(createdAt)
	c.UpdatedAt = repository.ParseTime(updatedAt)
	return &c, nil
}

func (r *chatRepository) SetSlowMode(chatID string, seconds int) error {
	_, err := r.db.Exec(
		`UPDATE chats SET slow_mode_seconds = ?, updated_at = ? WHERE id = ?`,
		seconds, time.Now().Format(time.RFC3339), chatID,
	)
	return err
}

func scanChatFull(row repository.Scanner) (*chatdomain.Chat, error) {
	var (
		c         chatdomain.Chat
		createdAt string
		updatedAt string
	)
	err := row.Scan(&c.ID, &c.Name, &c.Description, &c.AvatarURL, &c.Type, &c.CreatedBy, &c.SlowModeSeconds, &createdAt, &updatedAt)
	if err != nil {
		return nil, err
	}
	c.CreatedAt = repository.ParseTime(createdAt)
