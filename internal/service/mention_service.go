...
type MentionService interface {
	ExtractAndSaveMentions(messageID, content string) []*domain.MentionedUser
...
func (s *mentionService) ExtractAndSaveMentions(messageID, content string) []*domain.MentionedUser {
	matches := mentionRegex.FindAllStringSubmatchIndex(content, -1)
	if len(matches) == 0 {
		return nil
	}

	var mentioned []*domain.MentionedUser
	...
	for _, m := range matches {
		...
		mentioned = append(mentioned, &domain.MentionedUser{
			UserID:   user.ID,
			Username: username,
			Offset:   m[2],
			Length:   m[3] - m[2],
...
func (s *mentionService) GetMentionsByMessageID(messageID string) ([]*domain.Mention, error) {
