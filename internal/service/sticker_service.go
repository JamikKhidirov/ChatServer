...
func (s *stickerService) CreatePack(userID string, req *domain.CreateStickerPackRequest) (*domain.StickerPackWithStickers, error) {
	...
	return &domain.StickerPackWithStickers{
		StickerPack: *pack,
		Stickers:    []*domain.Sticker{},
	}, nil
}

func (s *stickerService) GetPacks() ([]*domain.StickerPackWithStickers, error) {
...
func (s *stickerService) GetMyPacks(userID string) ([]*domain.StickerPackWithStickers, error) {
...
func (s *stickerService) GetPackByID(id string) (*domain.StickerPackWithStickers, error) {
...
func (s *stickerService) AddSticker(packID, userID string, req *domain.AddStickerRequest) (*domain.Sticker, error) {
...
func (s *stickerService) GetLibrary(userID string) ([]*domain.Sticker, error) {
	stickers, err := s.stickerRepo.GetUserLibrary(userID)
	if err != nil {
		return nil, err
	}
	if stickers == nil {
		return []*domain.Sticker{}, nil
	}
	return stickers, nil
}

func (s *stickerService) enrichPacks(packs []*domain.StickerPack) ([]*domain.StickerPackWithStickers, error) {
	result := make([]*domain.StickerPackWithStickers, 0, len(packs))
	for _, p := range packs {
		...
		result = append(result, &domain.StickerPackWithStickers{
			StickerPack: *p,
			Stickers:    stickers,
		})
	}
