...
func NewVerificationService(verRepo repository.VerificationRepository, userRepo repository.UserRepository, emailSender *email.Sender, smsSender *sms.Sender) VerificationService {
...
func (s *verificationService) LoginSendPhoneCode(phone string) (string, error) {
	...
	code := generateCode()
	loginCode := &domain.PhoneLoginCode{
		ID:        uuid.New().String(),
		Phone:     phone,
		Code:      code,
		ExpiresAt: time.Now().Add(10 * time.Minute).Format(time.RFC3339),
